...
@foreach ($pengajuan as $d)
    <tbody data-toggle="collapse" data-target="#{{ $d->no_tiket }}" class=" {{ $d->urgent == 1 ? 'blink' : 'bg-info' }} clickable thead_pengajuan" aria-expanded="true">
        <tr>
            ...
            <th>
                ...
                <div class="custom-control custom-switch custom-switch-off-light custom-switch-on-warning">
                    <input type="checkbox" class="custom-control-input checkbox_urgent" name="urgent" id="urgent{{ $d->no_tiket }}" value="{{ $d->no_tiket }}" {{ $d->urgent == 1 ? 'checked' : '' }}>
            ...
            <th>
                ...
                <a href="{{ route('inputPengajuan',['tiket' => $d->no_tiket]) }}" class="btn btn-xs btn-secondary"><i class="fas fa-edit"></i></a> 
                <a class="btn btn-xs btn-secondary" target="_blank" href="{{ route('printPengajuan',['tiket' => $d->no_tiket]) }}"><i class="fas fa-print"></i> Print</a>
                @endif
